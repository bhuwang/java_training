...
public class PalindromProduct {
	private final static Logger LOGGER = Logger.getLogger(PalindromProduct.class.getName());
	...
	/**
	 * <p>This method determines the product of all the 3 digit numbers.</p>
	...
	public void getProduct(){
		for(int i=startIndex;i<=endIndex;i++){
			for(int j=startIndex;j<=endIndex;j++){
				int product=i*j;
				boolean istrue=checkPalindrom(product);
				if(istrue){
					if(greatest<product){
						num1=i;
						num2=j;
	...
	/**
	 * <p>This method reverses the number and checks whether its palindrom or not</p>
	 * @param product
