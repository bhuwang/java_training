...
public class UserNumInputImpl implements UserNumInput {
    ...
    private static final Logger LOGGER = Logger.getLogger(UserNumInputImpl.class.getName());
    ...
    public int getInputMin(Scanner scanner, int min) {
        ...
        while (true) {
            listLength = getInputInt(scanner);
            if (listLength < min) {
                LOGGER.log(Level.WARNING, "Please enter a number greater than {0} : ", min);
            } else {
                break;
            }
        }
    ...
    public int getInputMax(Scanner scanner, int max) {
        ...
        while (true) {
            listLength = getInputInt(scanner);
            if (listLength > max) {
                LOGGER.log(Level.WARNING, "Please enter a number less than {0} : ", max);
            } else {
                break;
            }
        }
