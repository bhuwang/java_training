...
public class LinkedListCollection {
	...
	private static void mergeElements() {
		...
		LOGGER.log(
			Level.INFO,
			"Elements in linked list one after merging from two are : {0}", one);
	}
	...
	private static void removeSecondWord() {
		...
		LOGGER.log(
			Level.INFO,
			"Elements in linked list two after removing every second word are : {0}",
			two);
	}
	...
	private static void removeAllWords() {
		...
		LOGGER.log(
			Level.INFO,
			"Elements in linked list one after removing all words in two are : {0}",
			one);
	}
